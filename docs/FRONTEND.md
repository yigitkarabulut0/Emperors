...
|---|---|---|---|
| Family | `family.gd` | `hero` | name, level, XP bar, stats (tap a cell to spend a point), gear tiles → chooser, EQUIP BEST, then the ledger: 11 Family upgrades, the Royal Treasury (deposit/withdraw), 8 estate holdings, the Legacy |
| Collect | `collect.gd` | `jobs` | today's three quests (tap to claim), fifteen job rows (six paintings cycle), optimistic COLLECT |
