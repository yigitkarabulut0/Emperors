...
> **Where this document no longer describes what ships.** The numbers below were
> derived against a 60 energy/hour budget; the game ships 120, with a pool that
> also grows with level (`energy.go` explains why). Everything downstream of the
> energy budget was re-derived from the shipped balance, and `balance/*.json` —
> generated by `scripts/gen-balance.py` — is the source of truth for all of it.
>
> | Section | This document says | What ships |
> |---|---|---|
> | §1.1 energy | 60 max, 60 s/point | 120 max +4/level, 30 s/point |
> | §2.2 job XP | `E × (1.40 + 0.02 × unlock)` | `max(2, round(E × 2.00 × 1.06^(n-2)))` |
> | §2.6 pacing | level 60 at day 91 | day 82 casual, 49 regular (`scripts/pace.py`) |
> | §13 the tax | base rate only, 8–12 h offline cap | base **plus 8 Territory holdings**, filling **the storehouse** (Wave 2): 8 h of the hourly rate, 12 min more per Tithe Barn level (12 h at its top), then it stops. The lord carries it to the purse, or to the vault less the deposit fee; it cannot be stolen while it waits. It replaced a credit to the purse on every request, uncapped (`service/storehouse.go`, `game/estates.Fill`) |
> | §13.3 passive share | 21 % base / 51 % maxed, per hour | measured per DAY instead, since tax runs 24 h and collecting does not |
> | §14.4 the ceiling | tax bounded by `24 × 1.045^60 × 24 h × 4.0` | holdings are outside that derivation; the `tax_income_bp` ceiling is now +60 %, enforced in `gameconfig.MaxTaxIncomeBP` |
> | §6.4 soldier level | soldiers level, and Train is the unbounded gold sink | **soldiers have no level**: stats are `type × tier`, fixed at recruitment. Train is gone and `/v1/army/train` answers 410 |
> | §5.2 roll variance | quality 85–115 %, masterwork ×1.15 | quality 95–105 %, masterwork ×1.08 — the old band swung 1.556× against a 1.35× tier gap, so **every adjacent tier could invert** |
> | §7.1 player stats | `10 + 2 × level` | `10 + 5 × ⌊level/5⌋` — the player is the only thing that grows with level, and it grows in steps |
> | §8.2 combat | `DMG_K 0.16`, HP `+2 %`/owner level | `DMG_K 0.20`, no per-level HP. With soldiers fixed, attack and HP no longer scale together, and the old pair stalled one fight in ten at the round cap |
>
> | §13.1 the tax | `24 × 1.045^L`, ~21 % of active | `40 × 1.055^L` plus holdings at 2.4× — **estates are the gold engine now**, running 130–330 % of what collecting earns. Tapping is the XP engine, and XP still gates every unlock |
>
> | §1.5 / §15.1 refills | 20/30/45/70/100/150, uncapped count | **three a day**, 20 / 30 / 45 (`progression.store.energy_refill_prices`), counted per local day in `players.refills_*`; a potion pays for one but still counts. Bought energy has a daily ceiling, whatever pays for it |
> | §0, §3 buckets | one cap per bucket | **two lanes**: the permanent bucket (collect +150 %, XP +100 %, regen +60 %) and a timed lane for whatever runs out (`BucketCollectIncomeTemp` +200 %, `BucketXPGainTemp` +100 %), filed by `economy.AddTemp`, applied together by `ApplyBucket` |
> | §15.4 diamonds | balances only | every movement is a row in `app.diamond_ledger` (reason, class, gross), a refund's shortfall is `players.diamond_debt` repaid from the next credit, and `itest` reconciles every balance against its rows |
> | §15.2 Patronage | +25 % `tax_income_bp`, auto-collect taxes | not built as written. The owner's rule is **money never buys gold or power**: Patronage and every paid product carry diamonds, tokens, cosmetics and comfort only, enforced by `gameconfig.CheckReward(r, paid)` (the expansion plan's Wave 1) |
> | §15 diamond income | level-ups and the daily calendar | Wave 1 added, each capped in the balance: the Royal Store's **daily deals** (a free gift; potions and charters for diamonds, `was` held by Validate to the store's own prices; a Splendour cosmetic 30 % off — `commerce.deals`), **promo codes** (no cosmetics, ≤ `rewards.promo.max_diamonds`), a **friend brought** to level 10 (100 + 100, `rewards.referral`), Royal Favour's daily gift and the Royal Stipend. Diamonds are sold now, so whatever diamonds buy keeps the paid rules |
> | §5 the market reroll | escalating price within a window, uncapped | still escalating within the 5-minute window, and **at most 20 a day** on the lord's own day (`items.shop.rerolls_per_day`, `players.shop_rerolls_*`): a reroll is the one place diamonds reach rarity, and diamonds are sold |
> | §15 the daily calendar | seven squares of diamonds (85 a week), reset on a miss | **28 squares** (`retention.calendar`, Wave 3): 340 diamonds over the cycle, the same 85 a week, weighted to the end, with gold and experience wages, flasks and cart writs between; days 7/14/21/28 are crowns (a title, a frame, rare gear, a crest + 100). One or two days missed breaks the run: mended for 20/50 diamonds or a Royal Pardon (one a cycle, two held at most), or begun anew; three missed and it starts over |
> | — the daily loop | not in this document | Wave 3 (`balance/retention.json`, all free, every grant through `grantBundle`): the **Tax Cart** (one every 4 h, three wait, published odds in bp, prize fixed by the lord and the cart's number), the **week's quests** (6, 40 points each, chests at 80/160/240), the **Golden Hour** (a run spending 40 % of the pool lights +100 % gold for 60 s on up to 30 % of it — the collect bucket's timed lane; gold only), the **Victory Road** (15 milestones, 435 diamonds), the steward's **guide**, and a **welcome back** letter. `scripts/pace.py` prints each source's pull on casual level 60: the loop together moves it 4.5 % on top of the daily gift's 3.7 % |
> | — live ops | not in this document | Wave 4 (`balance/liveops.json`). The **hourly event**: each hour rolls one row of a published table (10000 bp, 38 % quiet, never the same event twice running; `game/liveops.HourlyAt`, written down in `admin.hourly_events` when first rolled, settable a day ahead from the panel) and lasts its minutes from the top of the hour — Gold Rush +100 % gold 15 min, Scholar's Hour +100 % XP 15 min (both the timed lanes), Fortune's Favour +3000 luck, Quartermaster's Sale (the day's refills at half price, never more of them), Fresh Wares (one market reroll free), Busy Hands (the day's quests count double), the Royal Courier (a cart writ, once). **Festivals**: three days each, scheduled from the panel and frozen as they stood — +25 % gold (timed lane), +50 % renown (under the kingdom's daily cap) or −10 % at the market (under `MaxDiscountBP`); points from deeds capped 600 a festival day, five tasks, four milestones, places paid by letter. The **season**: 28 days from Monday 2026-09-14, the Royal Charter's 50 tiers on points capped 800 a season day — the free lane 170 diamonds, wages, flasks and cart writs; the royal lane (900 diamonds or the $7.99 Charter) 625 diamonds, potions, pardons, shields and looks only, validated as paid. The week's and the season's **boards** pay their top 50 by letter, and the season's renown names its **nobility** (worn through the season after). The **deeds**: 24 in four tiers, 1,800 diamonds in a lifetime. `scripts/pace.py`: the live ops move casual level 60 by 3.3 % on top of the daily loop (Scholar's Hour 2.7 % of it), within their own 5 %; `gen-balance.py` prints what they add to a lord who never pays (42.5 diamonds a week on the Charter, 10 on festivals) |
>
> | — rekabet | not in this document | Wave 5 (`balance/pvp.json`). The **Honour Arena**: an integer Elo ladder (K 32, the defender at half K, a published expected-score table so no two builds disagree), six leagues Bronze → Imperial where the last is a rating AND a place (≥1800 and the top fifty), five fights a day that nothing in the game may buy (Honor Hour's `arena_ticket` is the only way past, and it is never `paid_ok`), three free refreshes, and a half reset toward 1000 at each season. It moves a rating and **nothing else** — no gold, no energy, no shield, no renown — so `validate_pvp.go` refuses an arena grant carrying experience, energy or a boost, and the arena is outside `pace.py`'s budget by construction rather than by omission. It pays the day's first win a 45-energy wage and four rating chests (110💎 a season, ≈27.5/week, only to a lord who climbs). The **Bounty Board**: a price on a head for 1,000 / 5,000 / 20,000 gold plus a **burned** 20% — the wave's one gold sink — held in `app.bounties.remaining` and drawn down by claims of at most 3× the raid cap, so a large price is a week's hunt; a shield stops a hunt, and the hunted lord is told a price stands on them. Its leashes: no punching down, four claims a head a day then two hours, the same placer-and-claimer twice a week, an account 72 hours old. The **Throne**: every Monday the kingdom with at least three lords and the most renown *gained that week* (`app.kingdom_week`, because the standing figure is cumulative and would crown the same kingdom for ever) crowns its king for a reign; once in it he declares one sixty-minute edict that **the whole realm** feels, on the timed lane like any hourly event. `pace.py` measures the edict at +0.02% and prints it |
>
> | — krallık boss ve savaşları | not in this document | Wave 8 (`balance/boss.json`, `balance/war.json`). The **beast**: one per kingdom for 48 hours, six blows a member, each costing what a raid costs and each an ordinary `combat.Simulate` cut to eight rounds — what the lord did to it in them is their damage. Its health is the KINGDOM'S own Might times a calibrated share, per blow the kingdom has, so five lords and twenty meet the same siege; the share is measured, not guessed (`internal/game/boss/calibration_test.go` fights whole cycles of reference lords and holds the kill rate to 65–80 %). Three chests — struck, half an equal share of the damage, the kill — and the first three on the damage list take 10/6/4 diamonds and the Slayer's title, all by letter when the cycle closes. It **spends** 198 energy a cycle at level sixty to pay 85 wages of experience back, so like the campaign it slows a climb: what it pays is gold. The **war**: kingdoms drawn on a Friday evening by the Might of their best fifteen, never against more than 1.5× their own and never the same pair twice running; Saturday to Monday, three attacks a day. Nothing of a lord's is at stake — no gold, no energy, no shield, no cooldown, no renown per attack, not even their `action_seq` — and what moves is points: 10 × the Might ratio clamped to 0.5–2, a loss 2, a defence held 3, and a quarter of the win on a lord who has lost all three banners. The week's purse goes to every lord who rode out, the kingdom takes renown (which feeds the Throne) and experience, and the best lord of the week is the Warlord. `scripts/pace.py` measures the purse at 1.4 % of casual level 60, inside its own 2 % |
>
> | — the herald's advert | not in this document | Herald's Tidings (`commerce.ads`): a rewarded advert in the Royal Store, **2 diamonds, five a day, five minutes apart, from level 3** — 10 a day, 70 a week, to a lord who never pays. Two diamonds is derived, not picked: a rewarded impression at a ~$0.015 net eCPM against the store's own 60–85 diamonds per dollar makes an advert worth about one to one and a quarter, and a day's five worth roughly what a $0.99 pack's tenth is; a validator test holds the day to 12. The advert is FAIR by the same guard money is: `CheckReward(grant, paid=true)` refuses gold, experience, favour, gear and timed bonuses, so an advert buys what money buys and never power. It is not in `pace.py`'s free-daily budget, because it pays no energy and no experience -- only diamonds, the one thing the store sells. The realm ships with it **shut** (`EMPERORS_ADMOB_UNIT` unset): no section on the store, no ticket, and the callback still answers 200. Crown Patronage's `skip_ads` is **deliberately read by nothing**: the only advert is opt-in and pays the watcher, so honouring it would take 70 diamonds a week from the one lord who paid — `gameconfig/commerce.go` says so at the field |
>
> **Why the tax moved so far.** It is a function of LEVEL, and the level curve
> was slowed about 4× when the xp ramp came down to 1.06. Nothing about idle
> income changed, but a player at day 21 went from level 59 to level 42 and their
> passive income fell 61 % with them. Idle income has to be paced against the
> clock; pegging it to a curve that moved underneath it is what broke it.
>
> **The sink §14.5 counted on is gone.** Train was "effectively unbounded" and it
> absorbed most of a capped player's income. Reforge and re-recruiting remain;
> if the late game turns gold-rich, that is where to look first.
>
> The invariants in §0 are unchanged and still binding.
